...
A Helm chart for [order management]/[product order inventory] service
...
```bash
helm install my-release discobole-stable-repo/order-inventory
```
...
| image.repository | string | `"registry.gitlab.tech.orange/disco/disco-oda-components/disco-order-management/order-inventory/snapshot"` |  |
...
| nameOverride | string | `""` |  |
| nodeSelector | object | `{}` |  |
| orderInventory.service.authUserRole | string | `""` |  |
| orderInventory.service.kafka | string | `"kafka"` |  |
| orderInventory.service.mxHeap | string | `""` |  |
...
```console
$ helm install my-release -f values.yaml discobole-stable-repo/order-inventory
```
